...
document.querySelector("#register-form").addEventListener("submit", (e) => {
  ...
  if (attachInfos.length == 0) {
    result += `<input type='hidden' value='' name='missingImageDtos[0].path' >`;
    result += `<input type='hidden' value='' name='missingImageDtos[0].uuid' >`;
    result += `<input type='hidden' value='' name='missingImageDtos[0].imagename' >`;
  } else {
    attachInfos.forEach((obj, idx) => {
      // hidden 3개 => MovieImageDto 객체 하나로 변경(spring에서 대입해줌 알려주기만 하면 됨) list이기 때문에 []까지 입력
      result += `<input type='hidden' value='${obj.dataset.path}' name='missingImageDtos[${idx}].path' >`;
      result += `<input type='hidden' value='${obj.dataset.uuid}' name='missingImageDtos[${idx}].uuid' >`;
      result += `<input type='hidden' value='${obj.dataset.name}' name='missingImageDtos[${idx}].imagename' >`;
    });
  }
