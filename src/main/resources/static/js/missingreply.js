  ...
  fetch(`/missingreply/${missno}/all`)
    ...
    .then((data) => {
      ...
      data.forEach((review) => {
        ...
        result += `<p style="width:400px; text-align:justify;" >${review.text}</p>`;
...
replyForm.addEventListener("submit", (e) => {
  ...
  const missno = replyForm.querySelector("#missno");
  ...
  const missrno = replyForm.querySelector("#missrno");
  ...
  if (!missrno.value) {
    // 새 댓글 등록
    fetch(`/missingreply/${missno.value}`, {
      method: "post",
      headers: {
    ...
    // 댓글 수정
    fetch(`/missingreply/${missrno.value}`, {
      method: "put",
      headers: {
        "content-type": " application/json",
        "X-CSRF-TOKEN": csrfValue,
...
reviewList.addEventListener("click", (e) => {
  ...
  const missrno = btn.closest(".comment").dataset.missrno;
  console.log("missrno", missrno);
  ...
  if (btn.classList.contains("btn-outline-danger")) {
    ...
    fetch(`/missingreply/${missrno}`, {
      ...
      .then((data) => {
        ...
        replyForm.querySelector("#missrno").value = data.missrno;
